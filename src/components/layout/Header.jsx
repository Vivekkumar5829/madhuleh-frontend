...
export default function Header() {
  ...
  // close mobile on ESC
  useEffect(() => {
    const fn = e => { if (e.key === 'Escape') { setMobileOpen(false); setSearchOpen(false) } }
  ...
  const handleSearch = e => {
    ...
    if (q) { navigate(`/products?q=${encodeURIComponent(q)}`); setSearchOpen(false); setSearchVal('') }
  }
  ...
  return (
    <>
      ...
      <header className={`fixed top-0 inset-x-0 z-50 transition-all duration-300 ${
        scrolled ? 'bg-white/95 backdrop-blur-sm shadow-[0_1px_20px_rgba(26,13,0,0.08)]' : 'bg-cream-100'
      }`}>
        <div className="container">
          <div className="flex items-center justify-between h-16">
            ...
            <Link to="/" className="flex items-center gap-2.5 flex-shrink-0 group">
              <div className="w-8 h-8 bg-honey-500 rounded-xl flex items-center justify-center shadow-honey group-hover:scale-105 transition-transform">
                <span className="text-base leading-none">🍯</span>
              </div>
              <div>
                <p className="font-display font-bold text-lg text-bark-900 leading-none tracking-tight">Madhuleh</p>
                <p className="text-[9px] text-honey-600 font-semibold uppercase tracking-[0.15em] leading-none mt-0.5">Pure Honey</p>
              </div>
            </Link>
              ...
              {user ? (
                <div className="relative ml-1" ref={dropRef}>
                  <button onClick={() => setUserOpen(!userOpen)}
                    ...
                    <ChevronDown size={13} className={`text-bark-500 transition-transform ${userOpen ? 'rotate-180' : ''}`} />
                  </button>
                  ...
                  {userOpen && (
                    <div className="absolute right-0 top-full mt-2 w-52 bg-white rounded-2xl shadow-card-hover border border-gray-100 overflow-hidden z-50 animate-fade-up">
                      <div className="px-4 py-3 border-b border-gray-100 bg-honey-50">
                        <p className="font-bold text-bark-900 text-sm">{user.firstName} {user.lastName}</p>
                      ...
                      <div className="py-1">
                        <Link to="/orders" onClick={() => setUserOpen(false)}
                          ...
                          <Package size={15} className="text-honey-500" /> My Orders
                        </Link>
                        {isAdmin && (
                          <Link to="/admin" onClick={() => setUserOpen(false)}
                            className="flex items-center gap-2.5 px-4 py-2.5 text-sm text-bark-700 hover:bg-honey-50 transition-colors">
                            <User size={15} className="text-honey-500" /> Admin Panel
                          </Link>
                        )}
                ...
                <Link to="/auth" className="hidden sm:flex btn-primary btn-sm ml-2 text-sm">
                  Sign In
                </Link>
              )}
        ...
        <div className={`lg:hidden bg-white border-t border-gray-100 overflow-hidden transition-all duration-300 ${
          ...
          <div className="container py-4 space-y-1">
              ...
              <NavLink key={to} to={to} end={to === '/'}
                className={({ isActive }) =>
                  `block px-4 py-3 rounded-xl text-sm font-semibold transition-colors ${
                    isActive ? 'bg-honey-50 text-honey-600' : 'text-bark-700 hover:bg-gray-50'
                  }`
                }
              >
                {label}
            ...
            {!user && (
              <Link to="/auth" className="block w-full text-center btn-primary mt-3">
                Sign In / Register
              </Link>
            )}
            {user && (
              <>
                <Link to="/orders" className="block px-4 py-3 rounded-xl text-sm font-semibold text-bark-700 hover:bg-gray-50">
                  My Orders
                </Link>
                {isAdmin && (
                  <Link to="/admin" className="block px-4 py-3 rounded-xl text-sm font-semibold text-bark-700 hover:bg-gray-50">
                    Admin Panel
                  </Link>
                )}
                <button onClick={logout} className="block w-full text-left px-4 py-3 rounded-xl text-sm font-semibold text-red-600 hover:bg-red-50">
                  Sign Out
                </button>
              </>
            )}
      ...
      {/* ── Search overlay ──────────────────────────────────────────── */}
      {searchOpen && (
        <div className="fixed inset-0 z-[100] bg-bark-900/60 backdrop-blur-sm flex items-start justify-center pt-24 px-4"
          ...
          <div className="w-full max-w-2xl bg-white rounded-3xl shadow-2xl overflow-hidden animate-fade-up">
            ...
            <div className="px-5 py-3">
              ...
              <div className="flex flex-wrap gap-2">
                {['Multi Flora Honey', 'Tulsi Honey', 'Forest Honey', 'Royal Jelly', 'Comb Honey'].map(s => (
                  <button key={s}
                    onClick={() => { navigate(`/products?q=${encodeURIComponent(s)}`); setSearchOpen(false); setSearchVal('') }}
