...
const LINKS = {
  Shop: [
    ...
    { to: "/products?featured=true", l: "Featured" },
    { to: "/products?bestseller=true", l: "Bestsellers" },
  ],
  Company: [
    { to: "/about", l: "Our Story" },
    { to: "/blog", l: "Recipes & Blog" },
    { to: "/faq", l: "FAQs" },
...
export default function Footer() {
  ...
  const handleSubscribe = async (e) => {
    ...
    try {
      ...
      toast.error(
        err.response?.data?.message || "Already subscribed or try again",
      );
    } finally {
      setBusy(false);
    }
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-8">
            <div>
              ...
              <p className="text-cream-200/60 text-sm">
                Recipes, offers, and honey wisdom — delivered fresh.
              </p>
            </div>
            <form
      ...
      <div className="container py-14">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-10">
          <div className="lg:col-span-2">
            <Link to="/" className="flex items-center gap-2.5 mb-5">
              ...
              <div>
                ...
                <p className="text-[9px] text-honey-400 font-semibold uppercase tracking-[0.15em] mt-0.5">
                  Pure Honey
                </p>
              </div>
            </Link>
            <p className="text-sm text-cream-200/60 leading-relaxed mb-6 max-w-xs">
              Pure nature in every single drop. We source the finest raw honey
              from trusted beekeepers across India.
            </p>
            <div className="space-y-2.5 text-sm text-cream-200/60">
              <a
                href="tel:+919876543210"
                className="flex items-center gap-2.5 hover:text-honey-400 transition-colors"
              >
                <Phone size={14} className="text-honey-500" /> +91 9834343705
              </a>
              <a
                href="mailto:hello@madhuleh.com"
                className="flex items-center gap-2.5 hover:text-honey-400 transition-colors"
              >
                <Mail size={14} className="text-honey-500" />{" "}
          ...
          {Object.entries(LINKS).map(([group, links]) => (
            <div key={group}>
              <p className="text-xs font-black uppercase tracking-widest text-honey-500 mb-4">
                {group}
              </p>
              <ul className="space-y-2.5">
                {links.map(({ to, l }) => (
                  <li key={l}>
                    <Link
                      to={to}
                      className="text-sm text-cream-200/60 hover:text-honey-400 transition-colors"
                    >
                      {l}
                    </Link>
                  </li>
